...
export default function StatisticsTab() {
  ...
  if (error) {
    return <div className="text-red-500 text-center p-4">Error loading statistics: {error}</div>
  }
    ...
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <StatCard
          ...
          trend={`${formatPercentage(stats.byType[0]?.percentage)} of total`}
          color="#fb510f"
        />
        <StatCard icon={Target} label="Defect Types" value={stats.byType.length} color="#ecb662" />
      </div>

      {/* Enhanced Bar Chart */}
      <div className="bg-white rounded-xl border p-6 shadow-sm hover:shadow-md transition-all duration-300">
        <h3 className="text-lg font-semibold mb-6 text-[#0e4772]">Defect Distribution</h3>
